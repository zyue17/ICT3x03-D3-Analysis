...
DepositWithdrawRouter = APIRouter()
# Initializing DB
...
@DepositWithdrawRouter.post('/deposit')
async def deposit_money(request: Request):
    ...
    if transfer[0] != transfer[1] and float(transfer[2]) > 0:
        response = "Deposit Success"
        if response == "Deposit Success":
            try:
                # Parameterized query
                stmt = """UPDATE Vanguard.Accounts SET Balance = Balance + %s where AccountID = %s"""
                mycursor.execute(stmt, (transfer[2], transfer[1]))
                # mycursor.execute("UPDATE Vanguard.Accounts SET Balance = Balance + " +transfer[2]+" where AccountID='" + transfer[1] + "'")
                timestamp = int(time.time())

                # Parameterized query
                stmt = """INSERT INTO Vanguard.ExternalTransactions (value, Timestamp, account1, account2) VALUES (%s, %s, %s, %s)"""
                mycursor.execute(stmt, (str(float(transfer[2])), str(
...
@DepositWithdrawRouter.post('/withdraw')
    ...
    if transfer[0] != transfer[1] and float(balance) > float(transfer[2]):
        ...
        if response == "Withdraw Success":
            try:
                ...
                # Parameterized query
                stmt = """INSERT INTO Vanguard.ExternalTransactions (value, Timestamp, account1, account2) VALUES (%s, %s, %s, %s)"""
                mycursor.execute(stmt, (transfer[2], str(timestamp), transfer[0], transfer[1]))
                mydb.commit()
